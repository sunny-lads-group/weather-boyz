...
pub async fn get_all_policy_templates(pool: &Pool<Postgres>) -> Result<Vec<PolicyTemplate>, sqlx::Error> {
...
pub async fn get_policy_template_by_id(pool: &Pool<Postgres>, template_id: i32) -> Result<Option<PolicyTemplate>, sqlx::Error> {
...
pub async fn create_policy_template(pool: &Pool<Postgres>, template_data: &CreatePolicyTemplate) -> Result<PolicyTemplate, sqlx::Error> {
    info!("Creating new policy template: {}", template_data.template_name);
...
pub async fn get_policies_by_user_id(pool: &Pool<Postgres>, user_id: i32) -> Result<Vec<InsurancePolicy>, sqlx::Error> {
...
pub async fn get_policy_by_id(pool: &Pool<Postgres>, policy_id: i32) -> Result<Option<InsurancePolicy>, sqlx::Error> {
...
pub async fn create_insurance_policy(pool: &Pool<Postgres>, policy_data: &CreateInsurancePolicy) -> Result<InsurancePolicy, sqlx::Error> {
    info!("Creating new insurance policy: {} for user {}", policy_data.policy_name, policy_data.user_id);
...
pub async fn update_policy_status(pool: &Pool<Postgres>, policy_id: i32, new_status: &str) -> Result<bool, sqlx::Error> {
...
pub async fn get_conditions_by_policy_id(pool: &Pool<Postgres>, policy_id: i32) -> Result<Vec<PolicyCondition>, sqlx::Error> {
    ...
    debug!("Retrieved {} conditions for policy {}", conditions.len(), policy_id);
    Ok(conditions)
}

pub async fn create_policy_condition(pool: &Pool<Postgres>, condition_data: &CreatePolicyCondition) -> Result<PolicyCondition, sqlx::Error> {
    debug!("Creating new policy condition for policy id: {}", condition_data.policy_id);
...
pub async fn insert_weather_data(pool: &Pool<Postgres>, weather_data: &CreateWeatherData) -> Result<WeatherData, sqlx::Error> {
    debug!("Inserting weather data for station: {} at {}", weather_data.station_id, weather_data.recorded_at);
...
pub async fn get_weather_data_by_station_and_date_range(
    ...
    debug!("Fetching weather data for station {} from {} to {}", station_id, start_date, end_date);
    ...
    debug!("Retrieved {} weather data records for station {}", data.len(), station_id);
    Ok(data)
}
...
pub async fn get_claims_by_policy_id(pool: &Pool<Postgres>, policy_id: i32) -> Result<Vec<PolicyClaim>, sqlx::Error> {
...
pub async fn create_policy_claim(pool: &Pool<Postgres>, claim_data: &CreatePolicyClaim) -> Result<PolicyClaim, sqlx::Error> {
    info!("Creating new policy claim for policy id: {}", claim_data.policy_id);
...
pub async fn update_claim_status(pool: &Pool<Postgres>, claim_id: i32, new_status: &str, rejection_reason: Option<&str>) -> Result<bool, sqlx::Error> {
    ...
    let updated = result.rows_affected() > 0;
    if updated {
        info!("Successfully updated claim {} status to {}", claim_id, new_status);
    } else {
...
pub async fn get_policy_with_conditions(pool: &Pool<Postgres>, policy_id: i32) -> Result<Option<PolicyWithConditions>, sqlx::Error> {
...
pub async fn get_policy_with_claims(pool: &Pool<Postgres>, policy_id: i32) -> Result<Option<PolicyWithClaims>, sqlx::Error> {
    let policy = get_policy_by_id(pool, policy_id).await?;
...
mod tests {
    ...
    async fn test_get_all_policy_templates() {
        ...
        let template_names: Vec<&str> = templates.iter().map(|t| t.template_name.as_str()).collect();
    ...
    async fn test_get_policy_template_by_id() {
        ...
        let template = get_policy_template_by_id(&test_db.pool, first_template_id).await.unwrap();
    ...
    async fn test_get_policy_template_by_id_not_found() {
        ...
        let template = get_policy_template_by_id(&test_db.pool, 99999).await.unwrap();
    ...
    async fn test_create_policy_template() {
        ...
        let created_template = create_policy_template(&test_db.pool, &template_data).await.unwrap();
        
        assert_eq!(created_template.template_name, "Test Template");
        assert_eq!(created_template.policy_type, "test");
        assert_eq!(created_template.min_coverage_amount, Decimal::from_str("100.00").unwrap());
    ...
    async fn test_create_insurance_policy() {
        ...
        let created_policy = create_insurance_policy(&test_db.pool, &policy_data).await.unwrap();
        ...
        assert_eq!(created_policy.coverage_amount, Decimal::from_str("1000.00").unwrap());
    ...
    async fn test_get_policies_by_user_id() {
        ...
        let policies = get_policies_by_user_id(&test_db.pool, user_id).await.unwrap();
        ...
        create_insurance_policy(&test_db.pool, &policy_data).await.unwrap();
    ...
    async fn test_get_policy_by_id() {
        ...
        let retrieved_policy = get_policy_by_id(&test_db.pool, created_policy.id).await.unwrap();
    ...
    async fn test_update_policy_status() {
        ...
        let updated = update_policy_status(&test_db.pool, created_policy.id, "expired").await.unwrap();
        assert!(updated);
        
        // Verify the status was updated
        let retrieved_policy = get_policy_by_id(&test_db.pool, created_policy.id).await.unwrap().unwrap();
        assert_eq!(retrieved_policy.status, Some("expired".to_string()));
        
        // Test updating non-existent policy
        let not_updated = update_policy_status(&test_db.pool, 99999, "cancelled").await.unwrap();
        assert!(!not_updated);
    }
    ...
    async fn test_create_and_get_policy_conditions() {
        ...
        let policy = create_insurance_policy(&test_db.pool, &policy_data).await.unwrap();
        
        // Initially no conditions
        let conditions = get_conditions_by_policy_id(&test_db.pool, policy.id).await.unwrap();
        ...
        let created_condition = create_policy_condition(&test_db.pool, &condition_data).await.unwrap();
        ...
        assert_eq!(created_condition.threshold_value, Decimal::from_str("5.0").unwrap());
    ...
    async fn test_insert_weather_data() {
        ...
        let inserted_data = insert_weather_data(&test_db.pool, &weather_data).await.unwrap();
        
        assert_eq!(inserted_data.station_id, "test_station_001");
        assert_eq!(inserted_data.temperature, Some(Decimal::from_str("22.5").unwrap()));
        assert_eq!(inserted_data.humidity, Some(Decimal::from_str("65.0").unwrap()));
    ...
    async fn test_weather_data_upsert() {
        ...
        let first_insert = insert_weather_data(&test_db.pool, &weather_data).await.unwrap();
        ...
        let upserted_data = insert_weather_data(&test_db.pool, &updated_weather_data).await.unwrap();
        
        // Should be the same record (same ID) but with updated values
        assert_eq!(upserted_data.id, first_insert.id);
        assert_eq!(upserted_data.temperature, Some(Decimal::from_str("25.0").unwrap()));
        assert_eq!(upserted_data.humidity, Some(Decimal::from_str("75.0").unwrap()));
        assert_eq!(upserted_data.wind_speed, Some(Decimal::from_str("12.0").unwrap()));
    ...
    async fn test_get_weather_data_by_station_and_date_range() {
        ...
        for (month, day, temp) in dates {
            let weather_data = CreateWeatherData {
                ...
                recorded_at: PrimitiveDateTime::new(
                    time::Date::from_calendar_date(2024, 
                        if month == 1 { time::Month::January } else { time::Month::February }, 
                        day).unwrap(),
                    time::Time::from_hms(12, 0, 0).unwrap()
                ),
            ...
            insert_weather_data(&test_db.pool, &weather_data).await.unwrap();
        }
        ...
        assert_eq!(january_data[0].temperature, Some(Decimal::from_str("10.0").unwrap()));
        assert_eq!(january_data[1].temperature, Some(Decimal::from_str("15.0").unwrap()));
        assert_eq!(january_data[2].temperature, Some(Decimal::from_str("20.0").unwrap()));
    }
    ...
    async fn test_create_and_get_policy_claims() {
        ...
        // Initially no claims
        let claims = get_claims_by_policy_id(&test_db.pool, policy.id).await.unwrap();
        ...
        let created_claim = create_policy_claim(&test_db.pool, &claim_data).await.unwrap();
        
        assert_eq!(created_claim.policy_id, policy.id);
        assert_eq!(created_claim.claim_amount, Decimal::from_str("5000.00").unwrap());
    ...
    async fn test_update_claim_status_approved() {
        ...
        let claim = create_policy_claim(&test_db.pool, &claim_data).await.unwrap();
        assert_eq!(claim.claim_status, Some("pending".to_string()));
        
        // Approve the claim
        let updated = update_claim_status(&test_db.pool, claim.id, "approved", None).await.unwrap();
        assert!(updated);
    ...
    async fn test_update_claim_status_rejected() {
        ...
        // Reject the claim
        let rejection_reason = "Rainfall threshold not met according to official weather station data";
        let updated = update_claim_status(&test_db.pool, claim.id, "rejected", Some(rejection_reason)).await.unwrap();
        assert!(updated);
        ...
        assert_eq!(claims[0].rejection_reason, Some(rejection_reason.to_string()));
    ...
    async fn test_get_policy_with_conditions() {
        ...
        for condition_data in conditions {
            create_policy_condition(&test_db.pool, &condition_data).await.unwrap();
        }
        
        // Get policy with conditions
        let policy_with_conditions = get_policy_with_conditions(&test_db.pool, policy.id).await.unwrap();
        ...
        let rainfall_condition = policy_with_conditions.conditions.iter()
            .find(|c| c.condition_type == "rainfall").unwrap();
        assert_eq!(rainfall_condition.operator, "<");
        assert_eq!(rainfall_condition.consecutive_days, Some(10));
        
        let temp_condition = policy_with_conditions.conditions.iter()
            .find(|c| c.condition_type == "temperature_max").unwrap();
    ...
    async fn test_get_policy_with_claims() {
        ...
        for claim_data in claims {
            create_policy_claim(&test_db.pool, &claim_data).await.unwrap();
        }
        
        // Get policy with claims
        let policy_with_claims = get_policy_with_claims(&test_db.pool, policy.id).await.unwrap();
        ...
        assert_eq!(policy_with_claims.claims[0].claim_amount, Decimal::from_str("5000.00").unwrap());
        assert_eq!(policy_with_claims.claims[1].claim_amount, Decimal::from_str("3000.00").unwrap());
    }
    ...
    async fn test_get_nonexistent_policy_with_conditions() {
        ...
        let result = get_policy_with_conditions(&test_db.pool, 99999).await.unwrap();
